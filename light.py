...
def sortBySat(colors,DEBUG):
    '''
    Inputs an array of colors and returns the same array sorted by saturation with colors normalized to 0.5 brightness.
    '''
    ...
    # ['#724A7F' '#7F7671' '#7F7671' '#7F7871' '#787F79']
    #['#7F7772' '#7F7671' '#7F7870' '#75577F' '#347F70'] C
    #['#7F6A5F' '#7F695F' '#7F7860' '#7F746A' '#7F786D'] O
    ...
    for each in colors:
        ...
        # print(rgb,"RGB")
        ncs.append(rgb_to_hex( (int(r*255),int(g*255),int(b*255) )))
        sXb.append(col[1])
    # print(ncs)
    sXb = np.array(sXb)
    # print(sXb)
    arrInds = sXb.argsort()[::-1]
    if DEBUG:print(arrInds,"I")
    # colors = np.array(colors)
    colors=np.array(ncs)
    # ncs=np.array(colors)
    if DEBUG:print(colors,"C")
    orderedColors=colors[arrInds]
    if DEBUG:print(orderedColors,"O")
...
def setColor(color, bstick, start, end, inc=1):
    DELAY = 0.001
...
def screenColorGrabber(save=False, NUM_CLUSTERS=5, rgb=False,DEBUG=False):
   ...
   # print("Colors:",colors)
    ...
    colsXSat = sortBySat(colors,DEBUG)
    topColor = (colsXSat[0])
    if DEBUG:
...
def inBetween(color1:str, color2:str,many=10):
    color1 = Color(color1)
